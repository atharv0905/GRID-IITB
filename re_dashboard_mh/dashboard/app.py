...
def dashboard_layout():
    return dbc.Container(
        ...
        children=[
            ...
            dcc.Store(id="store_marker_click"),
            ...
            html.Div([
                ...
                dbc.Button("Logout", id="btn_logout", color="secondary", outline=True, size="sm", className="logout-btn"),
            ...
            dbc.Row([
                ...
                ], className="panel controls-panel"), md=3, lg=3, className="h-100"),
                ...
                dbc.Col(html.Div([
                    ...
                    dcc.Loading(type="circle", children=html.Div(
                        dl.Map(id="map", center=[22.0, 78.0], zoom=5,
                               style={"width": "100%", "height": "560px", "borderRadius": "14px"},
                               children=[
                                   dl.LayersControl(position="topright", collapsed=True, children=[
                                       dl.BaseLayer(dl.TileLayer(url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", attribution="Esri"), name="Satellite (Esri)", checked=True),
                                       dl.BaseLayer(dl.TileLayer(url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", attribution="OpenStreetMap"), name="Street (OSM)", checked=False),
                    ...
                    html.Div(("" if BOUNDARY_GEO else f"Boundary: {BOUNDARY_MSG}"), style={"opacity": 0.7, "marginTop": "6px", "fontSize": "12px"}),
                ], className="panel map-panel"), md=7, lg=7, className="h-00"),

                # Summary
                dbc.Col(html.Div([
                    html.H5("Summary", className="section-title"),
                    html.Div(id="kpi_cards", className="summary-stack"),
                ], className="panel"), md=2, lg=2, className="h-100"),
            ...
            html.Div([
                html.Div([
                    ...
                    html.Div([
                        ...
                        dbc.Button("Download CSV", id="btn_download", color="secondary", size="sm", className="dl-btn"),
...
def render_boundary(_):
    ...
    return dl.GeoJSON(
        data=BOUNDARY_GEO,
        options=dict(interactive=False, style=dict(color=BORDER_COLOR, weight=BORDER_WEIGHT, opacity=BORDER_OPACITY, fillOpacity=0)),
    )
...
def update_revision_dropdown(forecast_ui, plant_id, _n, picked_date, plants, current_rev):
    ...
    try:
        ...
        return ([], [{"label": "Latest", "value": "LATEST"}], "LATEST", None, None, None, "", None, None, "")
    all_runs = rlist or []
    dates = available_run_dates_ist(all_runs)
    if not dates:
        return (all_runs, [{"label": "Latest", "value": "LATEST"}], "LATEST", None, None, None, "No run dates available.", None, None, "")
    ...
    return (all_runs, opts, value, min_d, max_d, target_date.isoformat(), hint, min_d, max_d, f"Select range within: {min_d} → {max_d}")
...
def update_all(forecast_ui, plant_id, revision_choice, _n, marker_click, run_date, plants, runs_store, regions_store):
    ...
    try:
        if "timestamp" in df.columns and df["timestamp"].notna().any():
            ...
            t0 = start_ts - pd.Timedelta(hours=1)
    ...
    try:
        ...
        if reg:
            state_name = region_display_name(reg)
    except Exception:
        state_name = ""
    ...
    meta = html.Div([
        html.Div(f"{plant_name} | {state_name} • {plant.get('plant_type','')} • {float(plant['lat']):.4f},{float(plant['lon']):.4f}", className="meta-line1"),
        html.Div(f"Rev {rev} • Run t0 {run_t0_display} • Series {start_time} → {end_time}", className="meta-line2"),
    ])
    ...
    run_start_time = ""
    if "timestamp_str" in df.columns and df["timestamp_str"].notna().any():
        run_start_time = str(df["timestamp_str"].iloc[0])
    elif "timestamp" in df.columns and df["timestamp"].notna().any():
    ...
    kpis = html.Div([
        html.Div([
            ...
            html.Div(f"Avg Power: {fmt_mw(avg_total_p)} MW | Peak: {fmt_mw(peak_total_p)} MW" if avg_total_p is not None else "Avg Power: —", className="kpi-sub"),
            html.Div(f"Solar: {fmt_mw(avg_sol_p)} MW, Wind: {fmt_mw(avg_wind_p)} MW" if (avg_sol_p is not None or avg_wind_p is not None) else "", className="kpi-sub"),
        ], className="kpi-tile"),
        html.Div([
